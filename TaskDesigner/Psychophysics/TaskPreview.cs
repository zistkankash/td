...
namespace Psychophysics
{
    public partial class TaskPreview : XCoolForm.XCoolForm
    {
        ...
        public TaskPreview()
        {
            ...
            SaveOut_CB.Checked = Savetask; //A: related to saving chbx down the form
        ...
        {
            this.IconHolder.HolderButtons.Add(new XCoolForm.XTitleBarIconHolder.XHolderButton(Resource.file.GetThumbnailImage(20, 20, null, IntPtr.Zero), "File"));
            this.IconHolder.HolderButtons.Add(new XCoolForm.XTitleBarIconHolder.XHolderButton(Resource.add.GetThumbnailImage(20, 20, null, IntPtr.Zero), "Add"));
            this.IconHolder.HolderButtons.Add(new XCoolForm.XTitleBarIconHolder.XHolderButton(Resource.import.GetThumbnailImage(20, 20, null, IntPtr.Zero), "Load"));
            this.IconHolder.HolderButtons.Add(new XCoolForm.XTitleBarIconHolder.XHolderButton(Resource.save.GetThumbnailImage(20, 20, null, IntPtr.Zero), "Save"));
            this.IconHolder.HolderButtons.Add(new XCoolForm.XTitleBarIconHolder.XHolderButton(Resource.setting.GetThumbnailImage(20, 20, null, IntPtr.Zero), "Setting"));
            this.IconHolder.HolderButtons.Add(new XCoolForm.XTitleBarIconHolder.XHolderButton(Resource.help.GetThumbnailImage(20, 20, null, IntPtr.Zero), "Help"));

            this.IconHolder.HolderButtons[0].FrameBackImage = Resource.file.GetThumbnailImage(48, 48, null, IntPtr.Zero);
            this.IconHolder.HolderButtons[1].FrameBackImage = Resource.add.GetThumbnailImage(48, 48, null, IntPtr.Zero);
            this.IconHolder.HolderButtons[2].FrameBackImage = Resource.import.GetThumbnailImage(48, 48, null, IntPtr.Zero);
            this.IconHolder.HolderButtons[3].FrameBackImage = Resource.save.GetThumbnailImage(48, 48, null, IntPtr.Zero);
            this.IconHolder.HolderButtons[4].FrameBackImage = Resource.setting.GetThumbnailImage(48, 48, null, IntPtr.Zero);
            this.IconHolder.HolderButtons[5].FrameBackImage = Resource.help.GetThumbnailImage(48, 48, null, IntPtr.Zero);
        ...
        private void frmCoolForm_XCoolFormHolderButtonClick(XCoolForm.XCoolForm.XCoolFormHolderButtonClickArgs e)
        {
            ...
            {
                case 0: // A:file description button pushed

                    break;
                case 1: //A:Add button pushed
                    if (EnabledTask.Count == AllLevelProp.Count)
                    {
                        //A//TaskName++; 

                        Add_PB_Click(null, null);
                        EnabledTask.Add(0);     //A:what is this for?
                    }
                    break;
                case 4:   //A: setting button pushed
                    Setting SettingFrm = new Setting();
                    SettingFrm.FormClosing += delegate { this.Show(); };
                    this.Hide();
                    SettingFrm.Show();
                    break;
                case 3:  //A:save button pushed
                    ...
                    break;
                case 2: //A:load button pushed
                    LoadTaskFromFile();
                    /*OpenFileDialog theDialog = new OpenFileDialog();  //A
                    ...
                    if (theDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        for (int i = AllLevelName.Count; i > 0; i--)  //A:what the hell is he doing in this for? 
                        {
                    ...
                    /*AllLevelProp.Clear();
                        ...
                        using (var reader = new StreamReader(fs))
                        {
                            ...
                            for (int i = 0; i < AllLevelProp.Count; i++)
                            {
                                ...
                                UpdateData(i + 1);
                                UpdateComboBox(i + 1);*///A
                    break;
                case 5:    //A:help button pushed 
                    break;
                case 6:   //A: no idea!?!
                    break;
            }
        ...
        private void LoadTaskFromFile()
        {
            ...
            if (theDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    /* for (int i = AllLevelName.Count; i > 0; i--) //A
                     {
                         Label FrCountLB = Controls.Find("FramePerTask_LB" + (i), true).FirstOrDefault() as Label;
                         Task_Table.Controls.Remove(FrCountLB);
                         Label TotalTimeLB = Controls.Find("TotalTime_LB" + (i), true).FirstOrDefault() as Label;
                ...
                using (var reader = new StreamReader(fs))
                {
                    ...
                    for (int i = 0; i < AllLevelProp.Count; i++)
                    {
                        ...
                        Debug.Write(" Combo" + Combox.Name + "\n");
                        Combox.SelectedIndexChanged += new System.EventHandler(this.SelectTask_CB_SelectedIndexChanged);
                        this.Task_Table.Controls.Add(Combox, 1, this.Task_Table.RowCount - 1);*///A
                        var txbox = new TextBox();
                        txbox.Text = Convert.ToString(NumerRepeat[i]);
                        txbox.Name = "NumTrial_TB" + (Task_Table.RowCount - 1);
                        txbox.TextChanged += new System.EventHandler(this.NumTrial_TB_TextChanged);
                        ...
                        Task_Table.Controls.Add(new Label() { Text = "0", Name = "FramePerTask_LB" + (Task_Table.RowCount - 1) }, 4, this.Task_Table.RowCount - 1);
                        UpdateData(i + 1);
                        //A//UpdateComboBox(i + 1);
                       
                    }
                }
            }
        }
        //A:commented by meeeee! down with combo box!   
        ...
        {
            TaskName = Task_Table.RowCount;
            //add a new RowStyle as a copy of the previous one
            this.Task_Table.RowStyles.Add(new RowStyle(SaveRowStyle.SizeType, SaveRowStyle.Height));
            this.Task_Table.RowCount++;//A: why +1 then it -1 it at the column creation(next 2 line)?!?!?
            var namebox = new TextBox();
            namebox.Name = "NameTask_TB" + (Task_Table.RowCount - 1);//A: 1st column added
            namebox.TextChanged += new System.EventHandler(this.NameTask_TB_TextChanged);
            ...
            NumerRepeat.Add(1);
            //var Combox = new ComboBox();                     A:commented by meeeee! down with combo box!
            //Combox.Items.Add(" ");
            //Combox.Items.Add("Normal");
            ...
            Task_Table.Controls.Add(txbox, 2, this.Task_Table.RowCount - 1);
            Task_Table.Controls.Add(new Label() { Text = "0", Name = "TotalTime_LB" + (Task_Table.RowCount - 1) }, 3, this.Task_Table.RowCount - 1); //A:rowcount(default)=2 so -1  is for entering the last row 
            Task_Table.Controls.Add(new Label() { Text = "0", Name = "FramePerTask_LB" + (Task_Table.RowCount - 1) }, 4, this.Task_Table.RowCount - 1);
        }
        ...
        private void NameTask_TB_TextChanged(object sender, EventArgs e) //A:why this event starts at the begining of the psychopysics?! 
        {
            TextBox cmb = (TextBox)sender; //A:"casting" the object who created the event which is sender and it should be a text box

            String cmbName = cmb.Name;
            int index = (int)Char.GetNumericValue(cmbName[(cmbName.Length - 1)]); //A: it would be valid for nametask_TB0 to nametask_TB9----no more than that!
            Debug.Write("Helpppppp " + AllLevelName.Count + " \n");
            //A//AllLevelName[index] = cmb.Text;
            //A//AllLevelName.Add(cmb.Text);
